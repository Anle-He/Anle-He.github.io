...
import { existsSync, readdirSync, statSync, copyFileSync, mkdirSync, realpathSync } from 'fs'
    ...
    configureServer(server) {
      server.middlewares.use((req, _res, next) => {
        if (req.url && req.url.startsWith(URL_PREFIX + '/')) {
          try {
            const imagesRoot = realpathSync(resolve(root, CONTENT_IMAGES))
            const pathname = decodeURIComponent(new URL(req.url, 'http://localhost').pathname)
            const filePath = resolve(imagesRoot, pathname.slice(URL_PREFIX.length + 1))
            ...
            if (
              ...
              !relative(imagesRoot, realpathSync(filePath)).startsWith('..')
            ) {
              req.url = `/@fs/${filePath}`
            }
          } catch {
            // Ignore malformed or inaccessible image paths.
          }
        }
        next()
      })
